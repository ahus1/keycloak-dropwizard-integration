...
Methods that have neither and want to check if the user has sent a valid token can add a parameter _@Context SecurityContext context_ and check the logged-in user via _context.getUserPrincipal()_.
====
+
[source,java,indent=0]
